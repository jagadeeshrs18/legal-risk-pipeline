...
from legalrisk.document_builder import build_revised_text
...
@app.post("/report/<report_id>/revise")
def revise_report(report_id):
    original = load_report(report_id)
    if not original: return "Report not found", 404
    if original.get("original_report_id"):
        # Guard against chaining revisions of revisions indefinitely —
        # always revise from the original document, not an already-revised one.
        flash("This is already a revised agreement — go to the original report to generate a fresh revision.", "info")
        return redirect(url_for("view_report", report_id=report_id))

    revised_text = build_revised_text(original)
    ...
    new_token = uuid4().hex
    revised["report_id"] = new_token
    revised["filename"] = f"Revised — {original['filename']}"
    revised["category"] = original["category"]
    revised["created_at"] = datetime.now().strftime("%d-%m-%Y %H:%M")
    revised["category_confidence"] = original.get("category_confidence")
    revised["original_report_id"] = report_id
    revised["original_filename"] = original["filename"]
    revised["original_score"] = original["overall_score"]
    revised["original_level"] = original["level"]
    (REPORTS / f"{new_token}.json").write_text(json.dumps(revised, ensure_ascii=False, indent=2), encoding="utf-8")

    # Save the revised agreement's own plain text so it can be downloaded
    # as a standalone document, separate from the risk-report Word export.
    (REPORTS / f"{new_token}_revised_agreement.txt").write_text(revised_text, encoding="utf-8")

    # Link the original report forward to this revision too.
    original["revised_report_id"] = new_token
    (REPORTS / f"{report_id}.json").write_text(json.dumps(original, ensure_ascii=False, indent=2), encoding="utf-8")

    return redirect(url_for("view_report", report_id=new_token))


@app.get("/report/<report_id>/revised_agreement/word")
def revised_agreement_word(report_id):
    r = load_report(report_id)
    if not r or not r.get("original_report_id"): return "Revised agreement not found", 404
    text_path = REPORTS / f"{report_id}_revised_agreement.txt"
    if not text_path.exists(): return "Revised agreement text not found", 404

    doc = Document(); section = doc.sections[0]
    section.top_margin = section.bottom_margin = Inches(.65)
    doc.add_heading(f"Revised Agreement — {r.get('original_filename', '')}", 0)
    doc.add_paragraph(
        "This document was generated by applying the risk-reduction suggestions from the "
        "preliminary risk report to the original agreement. It is a drafting aid, not a "
        "finalised legal document — have it reviewed by a qualified legal professional "
        "before use."
    )
    for para in text_path.read_text(encoding="utf-8").split("\n\n"):
        clean = para.strip()
        if not clean: continue
        heading_line, _, body = clean.partition("\n")
        if body:
            doc.add_heading(heading_line, 2)
            doc.add_paragraph(body)
        else:
            doc.add_paragraph(clean)
    out = REPORTS / f"Revised_Agreement_{report_id}.docx"; doc.save(out)
    return send_file(out, as_attachment=True, download_name=out.name)
